...
},
{
  ...
  "body": " Entraînez-vous ! Symétries et projections   Exercice 1   On considère les vecteurs suivants dans :     On note la projection sur parallèlement à . Calculer             C'est ça ! Dans ce cas, , et comme est une famille libre, .    Non ! Ici, , et comme est une famille libre, .      C'est ça ! Dans ce cas, , et comme est une famille libre, .      Non ! Ici, , et comme est une famille libre, .     On note la matrice de dans la base de . Donner  ...le nombre de sur la diagonale de :  ...le nombre de sur la diagonale de :  ...le nombre de coefficients non nuls de :      C'est ça !  On a donc la matrice est   donc il y a 3 zéros sur la diagonale de .     Non !  On a donc la matrice est   donc il y a 3 zéros sur la diagonale de .       C'est ça !  On a donc la matrice est   donc il n'y a pas de -1 zéros sur la diagonale de .  On aurait pu s'en douter : comme est une projection, il n'existe aucun vecteur tel que , car sinon, on aurait .     Non !  On a donc la matrice est   donc il n'y a pas de -1 zéros sur la diagonale de .  On aurait pu s'en douter : comme est une projection, il n'existe aucun vecteur tel que , car sinon, on aurait .     C'est ça !  On a donc la matrice est   donc il y a un coefficient non nul dans .     Non !  On a donc la matrice est   donc il y a ul seuk coefficient non nul dans .      On note la matrice de dans la base canonique de . Donner  ...le nombre de sur la diagonale de :  ...le nombre de sur la diagonale de :  ...le nombre de coefficients non nuls de :      C'est ça !  On utilise la formule de changement de base : on note la base canonique de et , et on pose   C'est la matrice de passage de la base à la base , donc la matrice de dans la base canonique est   donc il n'y a aucun -1 sur la diagonale de .     Non, il n'y en a pas !  On utilise la formule de changement de base : on note la base canonique de et , et on pose   C'est la matrice de passage de la base à la base , donc la matrice de dans la base canonique est   donc il n'y a aucun -1 sur la diagonale de .       C'est ça !  On utilise la formule de changement de base : on note la base canonique de et , et on pose   C'est la matrice de passage de la base à la base , donc la matrice de dans la base canonique est   donc il n'y a aucun 0 sur la diagonale de .     Non, il n'y en a pas !  On utilise la formule de changement de base : on note la base canonique de et , et on pose   C'est la matrice de passage de la base à la base , donc la matrice de dans la base canonique est   donc il n'y a aucun 0 sur la diagonale de .     C'est ça !  On utilise la formule de changement de base : est la matrice de passage de la base canonique à la base , donc la matrice de dans la base canonique est   donc il y a 16 coefficients non nuls dans .     Non !  On utilise la formule de changement de base : est la matrice de passage de la base canonique à la base , donc la matrice de dans la base canonique est   donc il y a 16 coefficients non nuls dans .       Exercice 2   On utilise la même base qu'à l'exercice 1, et on note la symétrie par rapport à parallèlement à .    On note : c'est un s.e.v. de . Calculer               C'est ça !  Une symétrie est injective : deux vecteurs différents ne peuvent pas avoir le même symétrique. Donc .    Non, c'est 0 !  Une symétrie est injective : deux vecteurs différents ne peuvent pas avoir le même symétrique. Donc .      C'est ça !  Une symétrie est surjective : tous les vecteurs sont le symétrique d'un autre. On a donc .     Non, est de rang 4 !  Une symétrie est surjective : tous les vecteurs sont le symétrique d'un autre. On a donc .      C'est ça !  Puisque est la symétrie par rapport à , les vecteurs qui sont fixés par (c'est à dire, les vecteurs tels que ) sont ceux de . Et donc,   (la dernière égalité est vraie car est une famille génératrice de , et je vous laisse vérifier qu'elle est libre; c'est donc une base de )     Non, c'est 2 !  Puisque est la symétrie par rapport à , les vecteurs qui sont fixés par (c'est à dire, les vecteurs tels que ) sont ceux de . Et donc,   (la dernière égalité est vraie car est une famille génératrice de , et je vous laisse vérifier qu'elle est libre; c'est donc une base de )       On note la matrice de dans la base de . Donner  ...le nombre de dans la matrice :  ...le nombre de sur la diagonale de :  ...la trace de :      C'est ça !  On a donc la matrice est   donc il y a, en tout, 12 coefficients nuls.     Non, c'est 12 !  On a donc la matrice est   donc il y a, en tout, 12 coefficients nuls.       C'est ça !  On a donc la matrice est   donc il y a 2 fois le coefficient sur la diagonale de .     Non, il y en a 2 !  On a donc la matrice est   donc il y a 2 fois le coefficient sur la diagonale de .     C'est ça !  On a donc la matrice est   donc la trace est .     Non, la trace est nulle !  On a donc la matrice est   donc la trace est .      On note la matrice de dans la base canonique de . Donner  ...le nombre de dans la matrice :  ...le nombre de sur la diagonale de :  ...le déterminant de :      C'est ça !  On utilise la formule de changement de base : on note la base canonique de et , et on pose   C'est la matrice de passage de la base à la base , donc la matrice de dans la base canonique est   donc il y a 4 coefficients nuls dans la matrice .     Non, il y en a 4 !  On utilise la formule de changement de base : on note la base canonique de et , et on pose   C'est la matrice de passage de la base à la base , donc la matrice de dans la base canonique est   donc il y a 4 coefficients nuls dans la matrice .       C'est ça !  On utilise la formule de changement de base : on note la base canonique de et , et on pose   C'est la matrice de passage de la base à la base , donc la matrice de dans la base canonique est   donc il y a 1 zéro sur la diagonale de .     Non, il y a un coefficient non nul !  On utilise la formule de changement de base : on note la base canonique de et , et on pose   C'est la matrice de passage de la base à la base , donc la matrice de dans la base canonique est   donc il y a 1 zéro sur la diagonale de .     C'est ça !  On peut le calculer directement, et on trouve en effet que   Mais on n'est pas obligé de faire des calculs pour ça ! Le déterminant de est le même que celui de , car le déterminant ne dépend pas de la base utilisée, et il est facile d'obtenir en faisant le produit des coefficients diagonaux.     Non, c'est 1 !  On peut le calculer directement, et on trouve en effet que   Mais on n'est pas obligé de faire des calculs pour ça ! Le déterminant de est le même que celui de , car le déterminant ne dépend pas de la base utilisée, et il est facile d'obtenir en faisant le produit des coefficients diagonaux.      "
},
{
  "id": "sec--2-3",
  "level": "2",
...
},
{
  ...
  "number": "7.3",
  "title": "",
  "body": " On note la matrice de dans la base canonique de . Donner  ...le nombre de sur la diagonale de :  ...le nombre de sur la diagonale de :  ...le nombre de coefficients non nuls de :      C'est ça !  On utilise la formule de changement de base : on note la base canonique de et , et on pose   C'est la matrice de passage de la base à la base , donc la matrice de dans la base canonique est   donc il n'y a aucun -1 sur la diagonale de .     Non, il n'y en a pas !  On utilise la formule de changement de base : on note la base canonique de et , et on pose   C'est la matrice de passage de la base à la base , donc la matrice de dans la base canonique est   donc il n'y a aucun -1 sur la diagonale de .       C'est ça !  On utilise la formule de changement de base : on note la base canonique de et , et on pose   C'est la matrice de passage de la base à la base , donc la matrice de dans la base canonique est   donc il n'y a aucun 0 sur la diagonale de .     Non, il n'y en a pas !  On utilise la formule de changement de base : on note la base canonique de et , et on pose   C'est la matrice de passage de la base à la base , donc la matrice de dans la base canonique est   donc il n'y a aucun 0 sur la diagonale de .     C'est ça !  On utilise la formule de changement de base : est la matrice de passage de la base canonique à la base , donc la matrice de dans la base canonique est   donc il y a 16 coefficients non nuls dans .     Non !  On utilise la formule de changement de base : est la matrice de passage de la base canonique à la base , donc la matrice de dans la base canonique est   donc il y a 16 coefficients non nuls dans .    "
},
{
  "id": "sec--3-3",
  "level": "2",
  ...
  "number": "7.1",
  "title": "",
  "body": " On note : c'est un s.e.v. de . Calculer               C'est ça !  Une symétrie est injective : deux vecteurs différents ne peuvent pas avoir le même symétrique. Donc .    Non, c'est 0 !  Une symétrie est injective : deux vecteurs différents ne peuvent pas avoir le même symétrique. Donc .      C'est ça !  Une symétrie est surjective : tous les vecteurs sont le symétrique d'un autre. On a donc .     Non, est de rang 4 !  Une symétrie est surjective : tous les vecteurs sont le symétrique d'un autre. On a donc .      C'est ça !  Puisque est la symétrie par rapport à , les vecteurs qui sont fixés par (c'est à dire, les vecteurs tels que ) sont ceux de . Et donc,   (la dernière égalité est vraie car est une famille génératrice de , et je vous laisse vérifier qu'elle est libre; c'est donc une base de )     Non, c'est 2 !  Puisque est la symétrie par rapport à , les vecteurs qui sont fixés par (c'est à dire, les vecteurs tels que ) sont ceux de . Et donc,   (la dernière égalité est vraie car est une famille génératrice de , et je vous laisse vérifier qu'elle est libre; c'est donc une base de )     "
},
{
  "id": "sec--3-4",
  "level": "2",
  ...
  "number": "7.2",
  "title": "",
  "body": " On note la matrice de dans la base de . Donner  ...le nombre de dans la matrice :  ...le nombre de sur la diagonale de :  ...la trace de :      C'est ça !  On a donc la matrice est   donc il y a, en tout, 12 coefficients nuls.     Non, c'est 12 !  On a donc la matrice est   donc il y a, en tout, 12 coefficients nuls.       C'est ça !  On a donc la matrice est   donc il y a 2 fois le coefficient sur la diagonale de .     Non, il y en a 2 !  On a donc la matrice est   donc il y a 2 fois le coefficient sur la diagonale de .     C'est ça !  On a donc la matrice est   donc la trace est .     Non, la trace est nulle !  On a donc la matrice est   donc la trace est .    "
},
{
  "id": "sec--3-5",
  "level": "2",
  "url": "sec-.html#sec--3-5",
  "type": "Exercise",
  "number": "7.3",
  "title": "",
  "body": " On note la matrice de dans la base canonique de . Donner  ...le nombre de dans la matrice :  ...le nombre de sur la diagonale de :  ...le déterminant de :      C'est ça !  On utilise la formule de changement de base : on note la base canonique de et , et on pose   C'est la matrice de passage de la base à la base , donc la matrice de dans la base canonique est   donc il y a 4 coefficients nuls dans la matrice .     Non, il y en a 4 !  On utilise la formule de changement de base : on note la base canonique de et , et on pose   C'est la matrice de passage de la base à la base , donc la matrice de dans la base canonique est   donc il y a 4 coefficients nuls dans la matrice .       C'est ça !  On utilise la formule de changement de base : on note la base canonique de et , et on pose   C'est la matrice de passage de la base à la base , donc la matrice de dans la base canonique est   donc il y a 1 zéro sur la diagonale de .     Non, il y a un coefficient non nul !  On utilise la formule de changement de base : on note la base canonique de et , et on pose   C'est la matrice de passage de la base à la base , donc la matrice de dans la base canonique est   donc il y a 1 zéro sur la diagonale de .     C'est ça !  On peut le calculer directement, et on trouve en effet que   Mais on n'est pas obligé de faire des calculs pour ça ! Le déterminant de est le même que celui de , car le déterminant ne dépend pas de la base utilisée, et il est facile d'obtenir en faisant le produit des coefficients diagonaux.     Non, c'est 1 !  On peut le calculer directement, et on trouve en effet que   Mais on n'est pas obligé de faire des calculs pour ça ! Le déterminant de est le même que celui de , car le déterminant ne dépend pas de la base utilisée, et il est facile d'obtenir en faisant le produit des coefficients diagonaux.    "
},
{
